...
export default function Programmers() {
  ...
  const getNameOfFeatured = (id) => {
    ...
    const devName = progList.map(dev => {
      if (id === dev.id) {
        return dev.name;
      } 
    })

    const filteredName = devName.filter(name => {
      if (name !== undefined) {
        return name
      }
    })
      ...
      <div className='programmers'>
        {
          ...
          progList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */ setProgId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
      <div id='featured' style={style}>
        {
          ...
          progId
            ? `🎉 Let's celebrate ${getNameOfFeatured(progId)}! 🥳`
